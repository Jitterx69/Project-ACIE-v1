...
extern void fast_matrix_multiply_asm(float *A, float *B, float *C, int64_t M,
                                     int64_t N, int64_t K);

extern void fast_relu_asm(float *input, float *output, int64_t length);
extern void fast_sigmoid_asm(float *input, float *output, int64_t length);
extern void minkowski_metric_avx512(float *input, float *output,
                                    int64_t num_points);
extern void vector_mul_u64_avx512(uint64_t *A, uint64_t *B, uint64_t *C,
                                  int64_t N);
extern void vector_entropy_term_avx512(float *p, float *out, int64_t N);
extern void montgomery_mul_avx512(uint64_t *A, uint64_t *B, uint64_t *N,
                                  uint64_t *Out, uint64_t k0, int64_t count);

// C wrapper functions callable from Python via ctypes
void fast_matmul_wrapper(float *A, float *B, float *C, int64_t M, int64_t N,
                         int64_t K) {
  fast_matrix_multiply_asm(A, B, C, M, N, K);
}

void fast_relu_wrapper(float *input, float *output, int64_t length) {
  fast_relu_asm(input, output, length);
}

void fast_sigmoid_wrapper(float *input, float *output, int64_t length) {
  fast_sigmoid_asm(input, output, length);
}

void fast_minkowski_wrapper(float *input, float *output, int64_t num_points) {
  minkowski_metric_avx512(input, output, num_points);
}

void vector_mul_u64_wrapper(uint64_t *A, uint64_t *B, uint64_t *C, int64_t N) {
  vector_mul_u64_avx512(A, B, C, N);
}

void vector_entropy_wrapper(float *p, float *out, int64_t N) {
  vector_entropy_term_avx512(p, out, N);
}

void montgomery_mul_wrapper(uint64_t *A, uint64_t *B, uint64_t *N,
                            uint64_t *Out, uint64_t k0, int64_t count) {
  montgomery_mul_avx512(A, B, N, Out, k0, count);
}
...
static PyObject *py_fast_matmul(PyObject *self, PyObject *args) {
  ...
  fast_matrix_multiply_asm(A_data, B_data, C_data, (int64_t)A_dims[0],
...
static PyObject *py_fast_relu(PyObject *self, PyObject *args) {
  ...
  fast_relu_asm(in_data, out_data, length);
